...
class tx_paeproject_project extends tx_paeproject_dbrecord {
	...
	function getExceptions(){
			...
			if($this->data['uid'] != ""){
				$queryParts = array(
					...
					'WHERE' => "uid_foreign=".$this->data['uid']."",
				);
	 ...
	 *
	 * @return	0 if no children, else children count.
	 */
	...
	function delete(){
		...
		$sql = "DELETE FROM tx_paeproject_exception_project_elements_mm WHERE uid_foreign=".$this->data['uid'];
		if($GLOBALS['TYPO3_DB']->sql(TYPO3_db,$sql)){}
		else die("ERROR: class.tx_paeproject_project->delete(req2): " .$GLOBALS['TYPO3_DB']->sql_error());
		
		$sql = "DELETE FROM tx_paeproject_projectelement_administrators_mm WHERE uid_local=".$this->data['uid'];
		if($GLOBALS['TYPO3_DB']->sql(TYPO3_db,$sql)){}
		else die("ERROR: class.tx_paeproject_project->delete(req3): " .$GLOBALS['TYPO3_DB']->sql_error());
				
		$sql = "DELETE FROM tx_paeproject_projectelement_dependencies_mm WHERE uid_local=".$this->data['uid'];
		if($GLOBALS['TYPO3_DB']->sql(TYPO3_db,$sql)){}
		else die("ERROR: class.tx_paeproject_project->delete(req4): " .$GLOBALS['TYPO3_DB']->sql_error());
		
		$sql = "DELETE FROM tx_paeproject_projectelement_workers_mm WHERE uid_local=".$this->data['uid'];
		if($GLOBALS['TYPO3_DB']->sql(TYPO3_db,$sql)){}
		else die("ERROR: class.tx_paeproject_project->delete(req5): " .$GLOBALS['TYPO3_DB']->sql_error());
		
		$sql = "DELETE FROM tx_paeproject_projectelement_parent_mm WHERE uid_local=".$this->data['uid'];
		if($GLOBALS['TYPO3_DB']->sql(TYPO3_db,$sql)){}
	...
	function isEditingEnabled($fe_user_uid){
		...
		foreach($workers as $key=>$workers_scope){
			foreach($workers_scope as $key=>$worker){
