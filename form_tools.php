<?php
	...
	function writeInput($plugin, $llxmlName,$componentName, $value, $class){
		...
		return '<p><label for="'.$fieldName.'">'.$plugin->pi_getLL($llxmlName).'</label><input name="'.$fieldName.'" value="'.$value.'" class="'.$class.'" /></p>';
	}
	...
	function writeDate($plugin, $llxmlName,$componentName, $timestamp){
		...
		$datefield = '<p><label for="'.$fieldName.'">'.$plugin->pi_getLL($llxmlName).'</label>
		'.$plugin->pi_getLL('dd').'<input name="'.$fieldName_d.'" value="'.date('d',$timestamp).'" class="inputTiny" />
		'.$plugin->pi_getLL('mm').'<input name="'.$fieldName_m.'" value="'.date('m',$timestamp).'" class="inputTiny" />
		'.$plugin->pi_getLL('yyyy').'<input name="'.$fieldName_Y.'" value="'.date('Y',$timestamp).'" class="inputTiny" /></p>';
	...
	function writeDoubleList($plugin, $llxmlName, $componentName, $selectedValues, $selectedLabels, $availableValues, $availableLabels, $class, $formName, $size=7){
		...
		$baseKeys=array(); //keys of entries already selected on component display.
