		...
		function store(){
			...
			if($row[0]>0){
				...
				foreach($this->data as $key => $value){
					if($key != $this->data['uid']) $sql .= $key."='".$GLOBALS['TYPO3_DB']->quoteStr($value,$this->tableName).(($i<sizeof($this->data)-1)?"', " : (($i==sizeof($this->data)-1)? "'" : "'; "));
					$i++;
				}
				...
				reset($this->data);
				foreach($this->data as $key => $value){
					if($key == "crdate") $sql .= "crdate=".time().", ";
					else $sql .= $key."='".$GLOBALS['TYPO3_DB']->quoteStr($value,$this->tableName).(($i<sizeof($this->data)-1)?"', ":"';");
					$i++;
				}
